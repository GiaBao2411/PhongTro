...
# API Key OpenRouteService (Dùng chung cho cả Tìm kiếm & Dẫn đường)
...
# ==========================
# 1. CÁC VIEW HIỂN THỊ TRANG HTML
# ==========================

def home(request):
    # Lấy 6 phòng mới nhất
    latest_rooms = PhongTro.objects.order_by('-created_at')[:6]
    # Lấy 3 tin tức mới nhất (nếu có lỗi thì bỏ qua)
    try: 
...
def search_api(request):
    try:
        ...
        if is_rush_hour and vehicle != 'walk': 
            van_toc *= 0.6 
...
def news_list(request):
    try:
        all_news = TinTuc.objects.order_by('-ngay_dang')
    except:
        all_news = []
    return render(request, 'map_app/news_list.html', {'news': all_news})
